...
export function Heart(props) {
  ...
  useEffect(() => {
    ...
    group.current.children.forEach((mesh) => {
      ...
      // 랜덤 거리로 더 퍼지게
      const randomDistance = 4 + Math.random() * 4; // 4~8 사이 거리
      mesh.targetPosition = mesh.originalPosition
        .clone()
        .add(mesh.directionVector.clone().multiplyScalar(randomDistance));

      // 낙하에 사용할 속도, 낙하 지연시간, 랜덤 x/z 흔들림 추가
      mesh.fallVelocity = 0;
      mesh.fallDelay = Math.random() * 2; // 0 ~ 0.8초 사이 낙하 지연
      mesh.fallStarted = false;
      mesh.bounceCount = 0;
      mesh.bounceDamping = 0.6; // 튕김 강도 감소율
      mesh.fallWiggle = {
  ...
  const fallingStart = useRef(false); // 낙하 시작 여부
  const FLOOR_Y = -10;
  ...
  useFrame((_, delta) => {
    ...
    group.current.children.forEach((mesh) => {
      ...
      if (!fallingStart.current) {
        ...
        if (!mesh.fallStarted) {
          mesh.fallDelay -= delta;
          if (mesh.fallDelay <= 0) {
            mesh.fallStarted = true;
          }
          return;
        }

        // 중력
        mesh.fallVelocity += 9.8 * delta;
        mesh.position.y -= mesh.fallVelocity * delta;

        // 좌우 흔들림
        mesh.position.x += mesh.fallWiggle.x * delta;
        mesh.position.z += mesh.fallWiggle.z * delta;

        if (mesh.position.y < FLOOR_Y) {
          if (mesh.bounceCount < 3) {
            mesh.position.y = FLOOR_Y;
            mesh.fallVelocity *= -mesh.bounceDamping; // 반사 + 감쇠
            mesh.bounceCount += 1;

            // 감쇠 후 너무 작으면 멈춤
            if (Math.abs(mesh.fallVelocity) < 0.5) {
              mesh.fallVelocity = 0;
              mesh.position.y = FLOOR_Y;
              mesh.fallStarted = false; // 바운스 종료
            }
          } else {
            // 바닥 고정
            mesh.position.y = FLOOR_Y;
